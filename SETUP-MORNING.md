...
영어 주석만 있는 원본 GDPO.py를 이해하며 한글 주석을 붙일 때:

1. **탭 4 (본인 과거 노트북)** 확인 — "이런 스타일로 썼구나"
2. **VS Code (원본 GDPO.py)**에서 오늘 구간(예: 101~150줄) 읽기
3. **이해 안 되는 부분** → 탭 2(Colab)의 Gemini에게 질문:
   - 오른쪽 상단 ✨ **Gemini 아이콘** 클릭
   - 막히는 코드 복사/붙여넣기 → "이 코드를 한국어로 자세히 설명해줘"
   - Gemini 답변은 **완전 무료** (Colab 사용자 전부 사용 가능)
4. **탭 2 템플릿**에 본인만의 한글 주석 작성
5. 마지막 셀 실행 → GitHub 자동 push
