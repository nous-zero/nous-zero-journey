...
# --- Claude CLI 프롬프트 작성 ---
# 한글 주석은 ★/◎/※/→ 접두어로 분류 → Better Comments 확장이 색상 구분
$prompt = @"
박정훈(Python 초보자)이 학습용으로 읽을 수 있도록, 아래 Python 코드의 각 줄마다 한국어 주석을 추가해주세요.

한글 주석 접두어 규칙 (색상 구분용):
- `★` = 기본 한국어 설명 (금색) - 대부분의 줄에 사용
- `◎` = 핵심 개념/중요 포인트 (시안색) - 알고리즘 핵심이나 놓치면 안 되는 부분
- `※` = 주의/경고 (붉은색) - 함정, 메모리 이슈, 버그 위험
- `→` = 흐름/단계 설명 (연두색) - 다음 단계, 처리 순서

작성 규칙:
1. 원본 코드는 그대로 유지 (문자 하나 바꾸지 말 것, 원본 영어 주석도 유지)
2. 각 코드 줄 오른쪽에 # ★ 한국어 설명 형태로 추가
3. 이미 영어 주석이 있는 줄 아래에 # ★ 한국어 설명 추가 (별도 줄)
4. 전문 용어는 괄호로 쉬운 설명 병기 (예: 텐서는 행렬 형태의 숫자 묶음)
5. 핵심 개념은 ★ 대신 ◎ 사용, 주의사항은 ※, 처리 흐름은 → 사용
6. 설명 텍스트 없이 한글 주석이 추가된 Python 코드만 출력
7. 마크다운 코드 블록 없이 순수 Python 코드만 출력
8. 파일 접근 시도 금지 - 코드는 아래에 전부 포함되어 있음

예시:
`if seq_len > target_len:  # ★ 시퀀스 길이가 목표보다 크면`
`    length_score = max(0.0, ...)  # ◎ 핵심: 길이 페널티 계산 공식`
`    # ※ 주의: seq_len이 0이면 나눗셈 오류 발생 가능`
`    # → 다음: else 블록에서 길이가 짧을 때 처리`
